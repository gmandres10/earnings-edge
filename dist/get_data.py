"""Optional thin wrapper around ``yfinance`` (mirrors ``src.get_data`` API).

Methods return ``ValueError`` *instances* on failure rather than raising; callers
must check ``isinstance(..., ValueError)`` if they consume this helper.
"""
